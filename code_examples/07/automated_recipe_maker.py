...
def add_ingredient(receptacle, ingredient):
    print(
        f"Adding {ingredient.amount} {ingredient.units} of {ingredient.name} to {receptacle.name}"
    )
...
def slice(ingredient, *ingredients, thickness_in_inches, to_ignore=[]):
    names = [
        ing.name
        for ing in ([ingredient] + list(ingredients))
        if ing.name not in to_ignore
    ]
...
def chiffonade(ingredient):
    ...
    return Ingredient(
        "Chiffonade " + ingredient.name, ingredient.amount, ingredient.units
    )
...
def garnish(dishes, ingredient):
    print(
        f"Garnishing each dish with {ingredient.amount / len(dishes)} {ingredient.units} of {ingredient.name}"
    )
